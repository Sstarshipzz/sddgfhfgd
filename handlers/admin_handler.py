...
class AdminHandler:
    ...
    async def handle_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if not update.effective_user.id in self.bot.admin_ids:
            await query.message.edit_text("⛔️ Accès non autorisé")
            return
        ...
        try:
            if query.data == "admin_menu":
                ...
                if products:
                    message_text += "Produits dans cette catégorie:"
                else:
                    message_text += "Aucun produit dans cette catégorie"
    ...
    async def handle_product_creation(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if 'category_id' in product_data:
            if 'name' not in product_data:
                ...
                context.user_data['product_data'] = product_data
                await update.message.reply_text(
                    "💰 Écrivez le prix ou les informations de prix pour ce produit :"
                )
                return
    ...
    async def handle_media(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if context.user_data.get('creating_product'):
            ...
            new_product = {
                ...
                "name": product_data['name'],
                "description": product_data['description'],
                "price": product_data['price'],  # Le prix est maintenant une chaîne de caractères
